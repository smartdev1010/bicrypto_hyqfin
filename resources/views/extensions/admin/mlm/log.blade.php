        ...
        <table class="table table-hover custom-data-bs-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td data-label="{{ __('locale.Pair')}}" class="text-uppercase">
              ...
              <td data-label="{{ __('locale.Duration')}}">
                <div> {{ __('locale.Start')}}: <span
                    class="fw-bold">{{showDateTime($bot_log->start_date, 'd M, Y h:i:s')}}</span></div>
                <div> {{ __('locale.End')}}: <span
                    class="fw-bold">{{showDateTime($bot_log->end_date, 'd M, Y h:i:s')}}</span></div>
              </td>
              ...
              <td data-label="{{ __('locale.Action')}}">
                ...
                <button type="button" class="btn btn-icon btn-info btn-sm" data-bs-toggle="modal"
                  data-bs-target="#botSet" data-id="{{ $bot_log->id }}"
...
<div class="modal modal-slide-in fade" id="botSet">
  <div class="modal-dialog sidebar-sm">
    <form class="add-new-record modal-content pt-0" id="botcontract" action="{{ route('admin.bot.set') }}" method="POST"
      ...
      <div class="modal-body flex-grow-1">
        <div class="input-group">
          <input type="number" class="form-control" onkeyup="this.value = this.value.replace (/^\.|[^\d\.]/g, '')"
            required="" id="profit" name="profit" placeholder="Profit Amount">
          <span class="input-group-text text-dark" id="profit">{{ $general->cur_text }}</span>
        </div>
        <div class="d-flex justify-content-between align-items-center my-1">
          <div class="dropdown">
            <button class="btn btn-outline-warning dropdown-toggle" type="button" data-bs-toggle="dropdown"
              aria-expanded="false" id="botResult" name="botResult">{{ __('locale.Result')}}</button>
            <ul class="dropdown-menu dropdown-menu-end">
              <li><a class="dropdown-item"
                  onclick="$('#botResult').text($(this).text());$('#botcontract').find('input[name=result]').val($(this).data('result'));"
                  data-result="4">{{ __('locale.Win')}}</a></li>
              <li><a class="dropdown-item"
                  onclick="$('#botResult').text($(this).text());$('#botcontract').find('input[name=result]').val($(this).data('result'));"
                  data-result="5">{{ __('locale.Lose')}}</a></li>
              <li><a class="dropdown-item"
                  onclick="$('#botResult').text($(this).text());$('#botcontract').find('input[name=result]').val($(this).data('result'));"
                  data-result="6">{{ __('locale.Draw')}}</a></li>
            </ul>
          </div>
