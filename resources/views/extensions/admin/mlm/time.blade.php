            ...
            <tr>
              ...
              <td data-label="{{ __('locale.Action')}}">
                <a href="javascript:void(0)" data-id="{{ $bot_timing->id }}"
                  class="btn btn-icon btn-danger btn-sm removeModalBtn" data-bs-toggle="tooltip" data-bs-placement="top"
                  title="{{ __('locale.Remove')}}"
                  onclick="$('#removeModal').find('input[name=id]').val($(this).data('id'));$('#removeModal').modal('show');">
                  <i class="bi bi-trash"></i>
                </a>
                <a href="javascript:void(0)" data-id="{{ $bot_timing->id }}"
                  class="btn btn-icon btn-warning btn-sm editModalBtn" data-bs-toggle="tooltip" data-bs-placement="top"
          ...
          <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              <li><a class="dropdown-item"
                  ...
                  data-type="Min">{{ __('locale.Minute')}}</a></li>
              <li><a class="dropdown-item"
                  onclick="$('#duration_unitt').text($(this).text());$('#timeUnit').find('input[name=type]').val($(this).data('type'));"
                  data-type="Hour">{{ __('locale.Hour')}}</a></li>
              <li><a class="dropdown-item"
                  onclick="$('#duration_unitt').text($(this).text());$('#timeUnit').find('input[name=type]').val($(this).data('type'));"
                  data-type="Day">{{ __('locale.Day')}}</a></li>
            </ul>
          </div>
      ...
      <form action="{{ route('admin.bot.time.update') }}" method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="input-group mb-1">
            ...
            <input type="number" class="form-control" id="duration" name="duration" aria-label="Bot Duration"
