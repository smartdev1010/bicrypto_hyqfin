...
<div class="space-y-5 overflow-hidden">
  <div class="grid gap-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
    ...
    <livewire:components.charts.area element="verified_users" route="admin.users.all" title="Verified Users"
      tooltip="Verified Users" color="#55DD92" icon='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <livewire:components.charts.area element="unverified_users" route="admin.users.all" title="Unverified Users"
      tooltip="Unverified Users" color="#F05252" icon='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <div class="grid gap-5 grid-cols-2">
    ...
    <livewire:components.charts.transactions element="trades_report" route="admin.users.all" title="Trades Report"
      tooltip="Trades" color="#4467EF" icon='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <livewire:components.charts.monthly element="binary_trades" route="admin.users.all" title="Binary Trades Report"
      :tooltip="['Wins', 'Loses']" :colors="['#22c55e', '#ef4444']"
  ...
  <div class="grid gap-5 grid-cols-2">
    ...
    @endcan
    @can(['binary_practice_log', 'binary_trade_log'])
    <livewire:components.charts.monthly-transactions element="withdrawals" route="admin.users.all" title="Withdrawals"
      :tooltip="['Total', 'Fees', 'Completed', 'Pending']" :colors="['#9061F9', '#E74694', '#0E9F6E', '#E3A008']"
...
<div class="input-group mt-auto mx-auto">
  <a class="btn btn-primary w-50 btn-sm" href="{{ route('admin.notifications') }}">{{ __('Read all') }}</a>
  <a class="btn btn-danger w-50 btn-sm" href="{{ route('admin.notifications.clean') }}">{{ __('Remove all') }}</a>
</div>
</div>
@else
<div class="text-muted text-center" colspan="100%">
  <img height="128px" width="128px" src="https://assets.staticimg.com/pro/2.0.4/images/empty.svg" alt="" />
