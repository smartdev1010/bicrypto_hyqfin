...
@section('content')
    <div class="col-4 w-1/3 card text-center mx-auto">
        <div class="pt-5">
            <h1 class="card-title">{{ __('Providers Activator') }}</h1>
        </div>
        <div class="box">
            @php
                $license_code = null;
            ...
            @if (!empty($_POST['license']) && !empty($_POST['client']))
                ...
                @if ($activate_response['status'] != true)
                    <form action="{{ route('admin.provider.verify') }}" method="POST" class="space-y-5">
                        @csrf
                        <div class="px-5">
...
@push('breadcrumb-plugins')
    <a class="btn btn-outline-success" href="{{ route('admin.clean') }}"><i class="bi bi-speedometer2"></i>
        {{ __('Optimize') }}</a>
    <a class="btn btn-outline-secondary" href="{{ route('admin.provider.index') }}"><i class="bi bi-chevron-left"></i>
        {{ __('Back') }}</a>
@endpush
