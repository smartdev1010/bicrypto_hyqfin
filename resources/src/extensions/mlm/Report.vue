<template>
  <div>
    <div class="mt-5">
      <h1 class="text-center text-lg mb-2">Level Detail</h1>
      <div class="card relative overflow-x-auto">
        <VTable class="w-full text-left text-sm text-gray-500 dark:text-gray-400" :data="this.totalData"
          :key="this.totalData.length" :currentPage.sync="currentPage" :pageSize="5" :hideSortIcons="true"
          ...
          <template #head>
            <tr class="bg-gray-100 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
              ...
              <VTh sortKey="percentage" scope="col" class="py-3 px-6">
                <Col text="% of Income" />
              </VTh>
              <VTh sortKey="balance" scope="col" class="py-3 px-6">
          ...
          <template #body="{ rows }">
            <template v-if="this.totalData.length > 0">
              <tr v-for="row in rows" :key="row.id" class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                <td data-label="level" class="text-uppercase py-3 px-6">
                  {{ row.level }}
                </td>
                ...
                <td data-label="Percentage" class="py-3 px-6">
                  <toMoney :num="row.percentage" decimals="2" />
                </td>
    ...
    <div class="mt-5 mb-5 flex justify-center">
      <dropdown text="button" v-if="this.totalData.length > 0">
        ...
        <list-group :class="'text-start'">
          <div class="flex items-center border-b py-1 px-4 text-sm text-gray-900 dark:text-white">
            {{ $t("Select Level") }}
          </div>
          <list-group-item v-for="(data, index) in this.totalData" :key="index"
            ...
            <a class="block px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
    ...
    <h1 class="text-center text-lg mb-2">Level Details</h1>
    <div class="mt-5">
      <div class="card relative overflow-x-auto">
        <VTable class="w-full text-left text-sm text-gray-500 dark:text-gray-400" :data="this.currentData"
          :key="this.currentData.length" :currentPage.sync="currentPage1" :pageSize="5" :hideSortIcons="true"
          ...
          <template #head>
            <tr class="bg-gray-100 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
              ...
              <VTh sortKey="username" scope="col" class="py-3 px-6">
                <Col text="UserName" />
              </VTh>
      ...
      <VTPagination aria-label="Table navigation" v-model:currentPage="currentPage1" :total-pages="totalPages1"
        :boundary-links="true" class="flex items-center justify-between pt-4">
        <template #firstPage> {{ $t("First") }} </template>
...
export default {
  setup() {
    ...
    return { userStore, forexStore, investmentStore, walletsStore, mlmStore };
  },
  // component list
  components: {
    toMoney,
  ...
  methods: {
    ...
    async calculateCurrentData(downlines, depth) {
      ...
      for (let i = 0; i < downlines.length; i++) {
        ...
        await this.calculateCurrentData(downlines[i].downlines, depth + 1);
      }
    },
    async fetchData() {
    ...
    async calculateTotalInvestment(downlines, depth) {
      ...
      percentage /= 100;
      if (this.totalData.filter(data => (data.level === depth)).length === 0) {
        let data = {
          level: depth,
          count: 0,
          percentage,
      ...
      for (let i = 0; i < downlines.length; i++) {
        ...
        if (this.investmentStore.investment_logs_tmp.length > 0) {
          for (let j = 0; j < this.investmentStore.investment_logs_tmp.length; j++) {
            ...
            this.totalData[depth - 1].income += item.amount * percentage;
          }
        }
