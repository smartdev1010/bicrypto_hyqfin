<template>
    ...
    >
            ...
            <div class="xs:col-span-1 md:col-span-2 lg:col-span-3">
                <div class="mb-5 grid gap-5 sm:grid-cols-1 lg:grid-cols-2">
                    <div class="grid grid-cols-2 gap-5">
                        ...
                        >
                            <div class="card-body">
                                ...
                                <h2 class="fw-bolder">
                                    <div
                                        v-for="(investment, key) in this
                                            .totalInvestment"
                                        class="badge bg-danger mb-1"
                                    >
                                        <toMoney
                                            :key="key"
                                            :num="investment.amount"
                                            decimals="2"
                                        />
                                        <!-- {{ investment.wallet - 1 }} -->
                                        USDT
                    ...
                    <div class="card card-transaction">
                        ...
                        <div
                            v-if="
                                investmentStore.investment_logs.length > 0 &&
                                forexStore.forex_investment.length > 0
                            "
                            class=""
                            style="max-height: 280px; overflow-y: auto"
                        >
                            <div
                                v-for="item in investmentStore.investment_logs"
                                :key="item.id"
                                class="flex w-full justify-between rounded-t-lg border-gray-200 px-4 py-2 dark:border-gray-600"
                                :class="
                                    index !=
                                    investmentStore.investment_logs.length - 1
                                        ? 'border-b'
                                        : ''
                                "
                            >
                                <div class="flex items-center justify-start">
                                    ...
                                    <div>
                                        <span
                                            ...
                                            >{{
                                                forexStore.forex_investment.filter(
                                                    (investment) =>
                                                        investment.id ==
                                                        item.plan_id
                                                )[0].title
                                        ...
                                        <div class="text-xs">
                                            {{
                                                item.started_at.slice(0, 4) +
                                                "-" +
                                                item.started_at.slice(5, 7) +
                                                "-" +
                                                item.started_at.slice(8, 10)
                                            }}
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form @submit.prevent="startInvestment()">
                                ...
                                <div
                                    class="grid gap-5 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4"
                                >
                                    <dropdown text="button">
                                        <template #trigger>
                                            ...
                                            >
                                                <i
                                                    class="bi bi-sort-down-alt mr-2"
                                                ></i>
                                                <toMoney
                                                    v-if="
                                                        investment_wallet !=
                            ...
                            >
                                ...
                                <td class="py-3 px-6" data-label="Duration">
                                    <span class="badge bg-success">{{
                                        row.profited.getFullYear() +
                                        "-" +
                                        (row.profited.getMonth() + 1) +
                                        "-" +
        ...
        <div
            v-if="forexStore.account.length != 0"
            class="mt-5 grid gap-5 xs:grid-cols-1 md:grid-cols-3 lg:grid-cols-5"
            :key="forexStore.account.length"
        >
            <template
                v-for="(signal, index) in forexStore.account.signals"
                :key="index"
            >
                            ...
                            >
                                <template #trigger>
                                    ...
                                    >
                                        <i class="bi bi-sort-down-alt mr-2"></i>
                                        <toMoney
                                            v-if="withdraw_wallet != null"
                                            :num="withdraw_wallet.balance"
                                            decimals="2"
                                        />
                                        {{
                                            withdraw_wallet
    ...
    methods: {
        async fetchData() {
            ...
            if (
                ...
                for (
                    let i = 0;
                    i < this.investmentStore.investment_logs.length;
                    i++
                ) {
                    let item = this.investmentStore.investment_logs[i];
                    if (
                        this.totalInvestment.filter(
                            (investment) => investment.wallet == item.wallet_id
                        ).length == 0
                    )
                    ...
                    let currentDate = new Date(
                        this.investmentStore.currentDate
                    );
                    ...
                    let currentInvestment =
                        this.forexStore.forex_investment.filter(
                            (investment) => investment.id == item.plan_id
                        )[0];
                    ...
                    while (1) {
                        ...
                        if (deadline == 0) break;
                        if (startDate.getTime() >= currentDate.getTime()) break;
                        ...
                        this.investment_logs.push({
                            ...
                            status:
                                (currentDate.getTime() - tmpDate.getTime()) /
                                    (1000 * 3600 * 24) <=
                        ...
                        if (
                            this.totalProfit.filter(
                                (investment) =>
                                    investment.wallet == item.wallet_id
                            ).length == 0
                        )
                            this.totalProfit.push({
                                wallet: item.wallet_id,
                                amount:
                                    (item.amount * currentInvestment.roi) / 100,
                            });
                        else
                            this.totalProfit.filter(
                                (investment) =>
                    ...
                    if (
                        (profitedDate.getTime() - tmpDate.getTime()) /
                            (1000 * 3600 * 24) >=
                        currentInvestment.duration
                    ) {
                        await this.walletsStore.walletUpdate({
                            id: -this.investmentStore.investment_logs[i]
                                .wallet_id,
                            amount: item.amount,
                        });
                    ...
                    while (1) {
                        ...
                        currentProfit +=
                            (item.amount *
                                this.forexStore.forex_investment.filter(
                                    (investment) =>
                                        investment.id == item.plan_id
                                )[0].roi) /
                            100;
                        profitedDate.setDate(profitedDate.getDate() + 1);
                    }
                    await this.investmentStore.updateProfitedDate(
                        currentDate,
                        item.id
                    );
                    await this.walletsStore.walletUpdate({
        ...
        async startInvestment() {
            ...
            if (result !== false) {
                ...
                if (
                    this.totalInvestment.filter(
                        (investment) =>
                            investment.wallet == this.investment_wallet.id
                    ).length == 0
                )
                    this.totalInvestment.push({
                        wallet: item.wallet_id,
