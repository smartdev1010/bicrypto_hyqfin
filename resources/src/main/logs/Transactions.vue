<template>
    <div>
        <div class="mb-4 items-center justify-between xs:block xs:space-y-5 sm:flex sm:space-y-0">
            ...
            <Filter>
                <input v-model="filters.trx.value" type="text" class="filter-input" placeholder="Transaction ID" />
            </Filter>
        </div>
        <div class="card relative overflow-x-auto">
            <VTable :key="logStore.logs.transaction.length" v-model:current-page="currentPage"
                class="w-full text-left text-sm text-gray-500 dark:text-gray-400" :data="logStore.logs.transaction"
                ...
                <template #body="{ rows }">
                    <template v-if="logStore.logs.transaction.length > 0">
                        <tr v-for="row in rows" :key="row.id"
                            ...
                            <td data-label="Amount" class="py-4 px-6" :class="
                                'budget' + (row.trx_type == '+')
                                    ? 'text-success'
                                    : 'text-danger'
                            ">
        ...
        <VTPagination v-model:currentPage="currentPage" class="float-right flex items-center justify-between pt-4"
            :total-pages="totalPages" :boundary-links="true" :hide-single-page="true" :max-page-links="3">
