<template>
    <div ref="menu" class="select-wrap" @mouseleave="listHide">
        <button
            ref="trigger"
            class="select-trigger text-nowrap"
            :class="css"
            type="button"
            @click="onClick"
            @focus="onFocus"
        ...
        <div
            ...
            :class="{ visible: visible, top: top, bottom: bottom }"
        >
            <ul class="select-list">
                <li v-if="!optionsList.length">No options</li>
                <li
                    v-for="(o, i) of optionsList"
                    :key="o.value"
                    :class="{ active: o.active }"
                    @click.stop="changeOption(i, true)"
                >
                    {{ o.text }}
                </li>
            </ul>
...
export default {
    ...
    methods: {
        ...
        loadOptions() {
            ...
            if (slots.length > 0) {
                slots.forEach((e) => {
                    if (e.elm instanceof HTMLOptionElement) {
                        list.push(e);
                    }
                });
            }
