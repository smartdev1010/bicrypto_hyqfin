...
export default class Binance extends Bus {
    ...
    getSignedUrl(endpoint, params) {
        ...
        let qstr = this._ajax.serializeData(
            Object.assign({ recvWindow, timestamp }, params)
        );
        let signature = crypto
            ? crypto.HmacSHA256(qstr, this._apisecret).toString(crypto.enc.Hex)
            : "";
    ...
    fetchChartData(symbol, cb) {
        ...
        const endpoint = `${window.cors}${this._apiurl}/v3/klines?symbol=${symbol}&interval=1h&limit=168`;
        const prices = [];
    ...
    placeFakeOrder(symbol, type, side, price, quantity) {
        ...
        let orderOpen = this.fakeOrderData(
            symbol,
            type,
            side,
            price,
        ...
        let orderFilled = this.fakeOrderData(
            symbol,
            type,
            side,
            price,
        ...
        let orderCanceled = this.fakeOrderData(
            symbol,
            type,
            side,
            price,
        ...
        setTimeout(() => {
            this.emit("book_create", orderOpen);
        }, 300); // added to book
        setTimeout(() => {
            this.emit("user_order", orderResult);
    ...
    placeOrder(symbol, type, side, price, quantity, inforce) {
        ...
        if (type === "LIMIT")
            Object.assign(params, { price, timeInForce: inforce });
    ...
    fetchOpenOrders() {
        ...
        this._ajax.get(this.getSignedUrl("/v3/openOrders"), {
            ...
            success: (xhr, status, response) => {
                response.forEach((o) =>
                    this.emit("user_order", this.parseOrderData(o))
                );
                this.emit("user_data", true);
            },
            error: (xhr, status, error) => {
    ...
    extendStreamKey() {
        ...
        this._ajax.put(
            this.getPublicUrl("/v1/userDataStream", {
    ...
    startUserStream(listenKey, reconnect) {
        ...
        if (!ws)
            return this.emit(
                "user_fail",
                "Could not connect to user stream API endpoint."
            );
        ...
        ws.addEventListener("close", (e) => {
            ...
            this.stopTimer("user");
            this.checkReconnect("user", () =>
                this.startUserStream(listenKey, reconnect)
            );
        });
    ...
    startTickerStream(reconnect) {
        ...
        if (!ws)
            return this.emit(
                "ticker_fail",
                "Could not connect to live ticker stream API endpoint."
            );
        ...
        ws.addEventListener("close", (e) => {
            ...
            this.stopTimer("ticker");
            this.checkReconnect("ticker", () =>
                this.startTickerStream(reconnect)
            );
        });
    ...
    sockConnect(id, endpoint) {
        ...
        if (!("WebSocket" in window)) {
            this.emit(
                "sock_fail",
                "This web browser does not have WebSocket support."
            );
            return false;
        }
        try {
            ...
            let message = String(
                err.message ||
                    "WebSocket endpoint connection failed for (" +
                        endpoint +
                        ")."
            );
