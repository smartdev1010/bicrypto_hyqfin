    ...
    request(method, address, options) {
        ...
        let onSuccess =
            typeof options.success === "function"
                ...
                : function () {};
        let onDone =
            typeof options.done === "function" ? options.done : function () {};
        ...
        if (!method)
            return onError(
                xhr,
                0,
                "Must specify a request method to make a new request."
            );
        if (!address)
            return onError(
                xhr,
                0,
                "Must specify a request url address to make a new request."
            );
        ...
        xhr.addEventListener("load", (e) => {
            let { status, response, error } = this._responseParams(
                "load",
                xhr,
                type
            );
