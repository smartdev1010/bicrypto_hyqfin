...
$.autocompleteList = function(el, settings) {
		...
		function addItem(value, text) {
			...
			list.append($('<div class="">\
				<button type="button" class="remove-btn close text-muted" aria-label="Close"><div aria-hidden="true">&times;</div></button>\
				<span>' + text + '</span>\
...
$.tagsInput = function(el, settings)  {
		...
		function addItem(value, text) {
			...
			autocomplete.before($('<div class="badge border m-1"><span>' + text + '</span>\
				<a href="#" class="btn-link remove-btn"><i class="la la-times"></i></a>\
				<input name="' + settings.listName + '[' + settings.listId + ']" value="' + value + '" type="hidden">\
			 </div>'));
			 autocomplete.val("");
		};
