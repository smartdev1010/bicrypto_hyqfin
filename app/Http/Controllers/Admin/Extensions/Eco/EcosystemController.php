...
class EcosystemController extends Controller
{
    ...
    public function wallet($chain)
    {
        ...
        if ($wallet != null) {
            if ($chain == 'ETH') {
                $wallet->balance = json_decode($this->eco->ethGetAccountBalance($wallet->address))->balance;
                $wallet->trxs = json_decode($this->eco->ethGetAccountTransactions($wallet->address));
                $wallet->internal_trxs = json_decode($this->eco->ethGetAccountInternalTransactions($wallet->address));
            } else if ($chain == 'BSC') {
                $wallet->balance = json_decode($this->eco->bscGetAccountBalance($wallet->address))->balance;
            } else if ($chain == 'TRON') {
                try {
                    $wallet->balance = $this->eco->tronGetAccountBalance($wallet->address);
                } catch (\Throwable $th) {
                    //throw $th;
                }
            } else if ($chain == 'BNB') {
                ...
                $wallet->balance = json_decode($this->eco->polygonGetAccountBalance($wallet->address));
            } else if ($chain == 'KLAY') {
                $wallet->balance = json_decode($this->eco->klaytnGetAccountBalance($wallet->address))->balance;
            } else if ($chain == 'CELO') {
                $wallet->balance = json_decode($this->eco->celoGetAccountBalance($wallet));
            } else if ($chain == 'ALGO') {
                $wallet->balance = json_decode($this->eco->algorandGetAccountBalance($wallet->address))->balance;
            } else if ($chain == 'BTC') {
                $wallet->balance = json_decode($this->eco->btcGetBalance($wallet->address))->incoming;
            } else if ($chain == 'SOL') {
    ...
    {
        $wallet = json_decode($this->eco->generateWallet($chain));
        ...
        if (in_array($chain, ["BNB", "SOL"])) {
            ...
            $real_wallet->private_key = json_decode($this->eco->generatePrivateKey($chain, $wallet->mnemonic, '0'))->key;
            $real_wallet->address = json_decode($this->eco->generateAddressFromXPub($chain, $wallet->xpub, '0'))->address;
        }
        $real_wallet->save();
