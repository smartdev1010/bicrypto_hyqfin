...
(function() {
    ...
    this.tmpl = function tmpl(str, data) {
        ...
        var fn = /^[-a-zA-Z0-9]+$/.test(str) ?
            (cache[str] =
                cache[str] || tmpl(document.getElementById(str).innerHTML)) // Generate a reusable function that will serve as a template
...
Vvveb.Builder = {
    ...
    saveAjax: function(fileName, startTemplateUrl, callback, saveUrl) {
        ...
        startTemplateUrl = startTemplateUrl;
    ...
    setAjax: function(fileName, startTemplateUrl, callback, saveUrl) {
        file =
            fileName.file && fileName.file != "" ?
            fileName.file :
            Vvveb.FileManager.getCurrentFileName();
...
Vvveb.FileManager = {
    ...
    init: function(allowedComponents = {}) {
        this.allowedComponents = allowedComponents;
        this.treelist = $("#filemanager .treelist > ol").html("");

        $(this.treelist).on("click", "a", function(e) {
            e.preventDefault();
            return false;
        });

        $(this.treelist).on("click", "li[data-page] label", function(e) {
    ...
    addComponent: function(name, url, title, page) {
        $("[data-page='" + page + "'] > ol", this.treelist).append(
    ...
    loadPage: function(name, allowedComponents = false, disableCache = true) {
        $("[data-page]", this.treelist).removeClass("active");
        $("[data-page='" + name + "']", this.treelist).addClass("active");
    ...
    scrollBottom: function() {
        var scroll = this.treelist.parent();
...
Vvveb.Breadcrumb = {
    ...
    init: function() {
        this.treelist = $(".breadcrumb-navigator > .breadcrumb").html("");
